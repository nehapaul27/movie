...
aiBtn.addEventListener('click', () => {
  ...
  if (!query) {
    aiOutput.textContent = "Please type something like 'superhero', 'thriller', or 'space'.";
    return;
  }
  ...
  const matches = movies.filter(m => {
    ...
    if (
      (query.includes('action') && /(avengers|thor|batman|spider|black panther|doctor strange)/.test(title)) ||
      (query.includes('space') && /(interstellar|guardians|inception)/.test(title)) ||
      (query.includes('superhero') && /(avengers|thor|spider|batman|black panther|doctor strange)/.test(title)) ||
      (query.includes('mind') || query.includes('thriller')) && /(inception|joker|the batman)/.test(title)
    ) return true;
    return false;
  });

  if (matches.length > 0) {
    aiOutput.textContent = `🤖 Recommended for you: ${matches.map(m => m.title).join(', ')}`;
  } else {
    aiOutput.textContent = "No exact matches found — try another keyword!";
  }
});
